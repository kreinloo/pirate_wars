	...
	return {
		...
		*/
		fireAt: function (row, col) {
			...
			if (!activePlayer.isMoveUnique(row, col)) throw ERROR.REPEATED_MOVE;

			var opponentsField = opponentPlayer.getGameField();
			var result = opponentsField.fireAt(row, col);
			if (result == HIT.GAME_OVER) currentGame.incGamePhase();
		...
		*/
		opponentsAlignment: function () {
			...
			if (currentGame.areBothPlayersReady()) currentGame.incGamePhase();
			return true;
		},
