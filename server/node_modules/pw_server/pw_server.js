...
var PW_Server = function () {
	...
	this.userConnectHandler = function (data, socket, callback) {
		...
		this.db.getStmt("CHECK_PLAYER").get(data.name, function(err, row) {
				if (row == null) {
					self.db.getStmt("ADD_PLAYER").run(data.name, function(err, row) {
							self.userConnectHandler(data, socket, callback);
						}
					);
				}
				else {
	...
	*/
	...
	this.bindListeners = function (server) {
		...
		this.io.sockets.on("connection", function(socket) {
			...
			socket.on(CLIENT.AUTH, function (data, callback) {
				if (DEBUG.SERVER)
					util.log(CLIENT.AUTH + " " + JSON.stringify(data));
				server.userConnectHandler(data, socket, callback);
			});
			...
			socket.on(SCOREBOARD.REQUEST, function () {
				...
				server.scoreboard.requestHandler(socket);
			});
	...
	*/
	this.emitScoreboard = function (data, socket) {
		if (socket === undefined)
			this.io.sockets.emit(SCOREBOARD.DATA, data);
		else
