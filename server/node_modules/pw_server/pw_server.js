/*
	...
	Server exchanges data directly with clients using its io instance.
	It handles connections/disconnections, creating/deleting games, lobby chat,
	userlist requests. Server passes data regarding game to the GameServer
	and invokes Scoreboard instance if there's a scoreboard request. GameServer
	does not interact with players directly, they use Server's functions
	for that.
...
var PW_Server = function () {
	...
	this.fbGraph = "https://graph.facebook.com/me?access_token=";
	...
	this.userConnectHandler = function (data, socket, sanitized) {
		...
		if (data.token === undefined) {
			socket.emit(CLIENT.AUTH, {
				error : true,
				errorMsg : "No access token was provided!"
			});
			return;
		}
		request(this.fbGraph + data.token, function (err, res, body) {
			if (!err && res.statusCode == 200) {
				var result = JSON.parse(body);
				data.name = result.name;
				if (DEBUG.SERVER) {
					util.log(CLIENT.AUTH + ": " + data.name + " has connected.");
				}
				self.db.getStmt("CHECK_PLAYER").get(data.name, function(err, row) {
					if (row === undefined) {
	...
	this.initUserInfoHandler = function (socket) {
	...
	*/
	...
	this.bindListeners = function (server) {
		...
		this.io.sockets.on("connection", function(socket) {
			...
			socket.on(CLIENT.INIT_INFO, function () {
				if (DEBUG.SERVER)
					util.log(CLIENT.INIT_INFO);
				server.initUserInfoHandler(socket);
			});
