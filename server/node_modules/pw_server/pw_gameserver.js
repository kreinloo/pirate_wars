...
var Game = (function (gid, gs, p1, p2) {
	...
	this.call = function (data) {
		...
		switch(data.action) {
			...
			case GAME.ACTION.CONFIRM_ALIGNMENT:
				...
				if (!this.players[data.pid].getReady()) {
					...
					this.sendServerMessage(opponentID,
						this.players[data.pid].getName() + " has placed his/her ships.");
			...
			case GAME.ACTION.QUIT:
				var quitter = this.players[data.pid].getName();
				var winnerID = data.pid == this.pids[0] ? this.pids[1] : this.pids[0];
				this.endGame({
					msg: quitter + " has quit the game!",
					winner : winnerID,
					loser : data.pid,
					replay : false
				});
	...
	this.endGame = function (data) {
		...
		this.sendEventResult({
			...
			msg : data.msg,
			winner : this.pids[0] == data.winner ? true : false,
			replay : data.replay
		});
		this.sendEventResult({
			pid : this.pids[1],
			action : GAME.ACTION.GAME_OVER,
			msg : data.msg,
			winner : this.pids[1] == data.winner ? true : false,
			replay : data.replay
		});
