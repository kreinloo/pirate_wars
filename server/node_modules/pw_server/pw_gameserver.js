...
GameServer = (function (server) {
	...
	this.endGame = function (gid) {
		...
		this.server.getDB().getStmt("ADD_GAME").run(
			this.activeGames[gid].activePlayer,		// winner
			this.activeGames[gid].passivePlayer		// loser
		);
			...
			case GAME.ACTION.FIREAT:
				...
				if (result === ERROR.INVALID_MOVE) {
					...
					if (result === HIT.GAME_OVER) {
						...
						this.endGame({ msg: winner + " has won the battle!"});
					}
...
var TestTable = (function () {
	...
	this.confirmShipCount = function () {
		var v = 0;
		var i, j;
		for (i = 0; i < 10; i++) {
			for (j = 0; j < 10; j++) {
	...
	this.validateTable = function (data) {
